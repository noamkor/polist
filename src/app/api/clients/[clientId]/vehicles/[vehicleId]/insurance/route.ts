...
export async function GET(
  ...
  const records = await prisma.vehicleInsurance.findMany({
    where: { vehicleId },
...
export async function POST(
  ...
  const record = await prisma.vehicleInsurance.create({
    data: {
      year: data.year,
...
export async function PUT(request: NextRequest) {
  ...
  const record = await prisma.vehicleInsurance.update({
    where: { id },
    data: {
      year: data.year,
      provider: data.provider || null,
