...
export async function GET(
  ...
  const records = await prisma.homeInsurance.findMany({
    where: { homeId },
...
export async function POST(
  ...
  const record = await prisma.homeInsurance.create({
    data: {
      year: data.year,
...
export async function PUT(request: NextRequest) {
  ...
  const record = await prisma.homeInsurance.update({
    where: { id },
    data: {
      year: data.year,
      provider: data.provider || null,
...
export async function DELETE(request: NextRequest) {
  ...
  await prisma.homeInsurance.delete({ where: { id } });
