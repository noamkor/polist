...
export async function GET(
  ...
  const records = await prisma.healthInsurance.findMany({
    where: { healthPolicyId: policyId },
...
export async function POST(
  ...
  const record = await prisma.healthInsurance.create({
    data: {
      year: data.year,
...
export async function PUT(request: NextRequest) {
  ...
  const record = await prisma.healthInsurance.update({
    where: { id },
    data: {
      year: data.year,
      policyNumber: data.policyNumber || null,
