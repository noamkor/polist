...
export function HealthManager({ clientId }: { clientId: string }) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    if (res.ok) {
      toast("פוליסת הבריאות נוספה בהצלחה");
      fetchPolicies();
      setShowAdd(false);
  ...
  async function handleDelete() {
    if (!deleteId) return;
    setDeleting(true);
    const res = await fetch(`/api/clients/${clientId}/health/${deleteId}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <Button onClick={() => setShowAdd(true)}>{labels.newHealthPolicy} +</Button>
      </div>

      {policies.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">אין פוליסות בריאות</div>
      ) : (
            ...
            <div key={policy.id} className="bg-card rounded-xl shadow-sm border border-border-light overflow-hidden">
              <div className="px-5 py-4 flex items-center justify-between cursor-pointer hover:bg-accent"
                ...
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="sm" title={labels.delete} aria-label={labels.delete} onClick={(e) => { e.stopPropagation(); setDeleteId(policy.id); }}>
              ...
              {expandedId === policy.id && (
                <div className="px-5 pb-5 border-t border-border-light pt-4 bg-background dark:bg-foreground/5">
                  <InsuranceYearRecords
                    apiUrl={`/api/clients/${clientId}/health/${policy.id}/insurance`}
                    clientId={clientId}
                    assetId={policy.id}
      ...
      <Modal open={showAdd} onClose={closeAddModal} title={labels.newHealthPolicy}>
